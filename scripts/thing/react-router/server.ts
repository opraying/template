/**
 * SOURCE: https://github.com/mjackson/remix-the-web/blob/main/packages/node-fetch-server/README.md
 */
 ...
 */
export type ErrorHandler = (error: unknown) => void | Response | Promise<void | Response>
 ...
 * ```
 */
export function createRequestListener(handler: FetchHandler, options?: RequestListenerOptions): http.RequestListener {
  const onError = options?.onError ?? defaultErrorHandler

  return async (req, res) => {
    const protocol = options?.protocol ?? ('encrypted' in req.socket && req.socket.encrypted ? 'https:' : 'http:')
...
function defaultErrorHandler(error: unknown): Response {
  console.error('Server Request handle', error)
