...
export class SQLiteAdapter implements Adapter {
  ...
  public async deleteSession(sessionId: string): Promise<void> {
    await this.controller.execute(`DELETE FROM ${this.escapedSessionTableName} WHERE id = ?`, [sessionId])
  }
  ...
  public async setSession(databaseSession: DatabaseSession): Promise<void> {
    ...
    await this.controller.execute(
      `INSERT INTO ${this.escapedSessionTableName} (${columns.join(', ')}) VALUES (${placeholders.join(', ')})`,
      values,
    )
  }

  public async updateSessionExpiration(sessionId: string, expiresAt: Date): Promise<void> {
    await this.controller.execute(`UPDATE ${this.escapedSessionTableName} SET expires_at = ? WHERE id = ?`, [
  ...
  public async deleteExpiredSessions(): Promise<void> {
    await this.controller.execute(`DELETE FROM ${this.escapedSessionTableName} WHERE expires_at <= ?`, [
