import { useAppReload, useAppUpdateAvailable } from '@xstack/app/app-life-cycle/provider'
...
export function UpdateAvailable({ cancel, className, visible }: Props) {
  ...
  return (
    <div
      className={cn('flex min-h-[40px] items-center justify-between py-1 px-4 border rounded bg-background', className)}
    >
      <div className="flex items-center space-x-1.5 pr-1">
        <div className="w-2.5 h-2.5 bg-secondary rounded-full" />
        <div>Ready to update.</div>
      </div>
...
export function UpdateAvailableTips({
  ...
  }, [visible, className, cancel, toast])
