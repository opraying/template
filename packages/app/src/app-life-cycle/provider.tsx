import type { AppLifecycleHook, InstallPromptType, UserChoice } from '@xstack/app/app-life-cycle/life-cycle'
import { defaultValue, getAppLifecycleHook, loop } from '@xstack/app/app-life-cycle/life-cycle'
...
export function useAppLifecycleContext() {
  const ctx = use(AppLifecycleContext)
...
export function AppLifecycleProvider({ children }: { children?: React.ReactNode }) {
  ...
  return <AppLifecycleContext value={state}>{children}</AppLifecycleContext>
}
